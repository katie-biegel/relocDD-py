...
from utility.jackknife import removeevent,removestation

# Main HYPODD Function
def hypoDD(log,hinputs,outfol='outputs',reloctype=1,fileout=0,hypoDDdata=[],
           iboot=None,bootres=[],dtboot=[],jks=-9,jke=-9):
    """
    ...
    if iboot:
        fn_loc = os.path.join(iboot,'tradouts','hypoDD.loc')
        fn_reloc = os.path.join(iboot,'tradouts','hypoDD.reloc')
        fn_res = os.path.join(iboot,'tradouts','hypoDD.res')
        fn_stares = os.path.join(iboot,'tradouts','hypoDD.stares')
        fn_srcpar = os.path.join(iboot,'tradouts','hypoDD.srcpar')
    ...
    amcusp = np.zeros(5000,dtype='int')
    ...
    for icl in range(ibeg,iend):
        ...
        if ndt==0:
            print('(hypoDD) Cluster %i has no data.' % icl)
            continue
        ...
        if iboot and jks==-9 and jke==-9:
            #if reloctype!=2:
            log.write('\n(hypoDD) Bootstrapping is running.  Randomly re-sampling data with replacement.\n')
            print('(hypoDD) Bootstrapping is running.  Randomly re-sampling data with replacement.')
            dt_dt = dtboot + bootres
            log.write('(hypoDD) Saving data.')
            np.savetxt('%sdt_postresample.txt',dt_dt)
        elif iboot and jks!=-9 and jke==-9:
            log.write('\n(hypoDD) Removing station for jacknife.\n')
            print('(hypoDD) Removing station for jacknife.')
            [sta_lab,sta_lat,sta_lon,
             dt_sta1,dt_sta2,dt_dt,dt_qual,dt_c1,dt_c2,dt_idx,dt_ista1,dt_ista2,
             dt_ic1,dt_ic2,dt_offse,dt_offss,nsta,ndt,ncc,nct,nccp,
             nccs,nctp,ncts]= removestation(log,reloctype,jks,
            ...
            log.write('\n(hypoDD) Removing event for jacknife.\n')
            print('(hypoDD) Removing event for jacknife.')
            [ev_date,ev_time,ev_cusp,ev_lat,ev_lon,ev_dep,ev_mag,ev_herr,ev_zerr,ev_res,
             dt_sta1,dt_sta2,dt_dt,dt_qual,dt_c1,dt_c2,dt_idx,
             dt_ista1,dt_ista2,dt_ic1,dt_ic2,dt_offse,dt_offss,
             nev,ndt,ncc,nct,nccp,
             nccs,nctp,ncts]= removeevent(log,reloctype,jke,
        ...
        log.write('(hypoDD) Cluster centroid at: %10.6f  %11.6f  %9.6f \n' % (sdc0_lat,sdc0_lon,sdc0_dep))
        ...
        while iteri<maxiter:   # while iteri (current iteration) less than max number of iterations
            ...
            if idata==3 and (ncc==0 or nct==0):
                print('Lack of data. NCC = %i and NCT = %i' % (ncc,nct))
                break
            ...
            for i in range(nsrc):
                if (src_dep[i]+src_dz[i]/1000.)<0.: # If depth negative
                    ...
                    k += 1
                    if k>3000:  # Trigger exception if there are too many airquakes.
                                # Indicates poor data quality or poorly constrained inversion              
                        raise Exception('(hypoDD) >>> More than 2000 air quakes.')
            ...
            """
            if fileout==0 and not iboot and jke==-9 and jks==-9:
                if isolv==1:
                    acond=-9
    ...
    -----
    ########## KB Note: Added to incorporate and understand absolute location information.
    -----
    ...
    [tmp_ttp_old,tmp_tts_old,
     tmp_xp,tmp_yp,tmp_zp] = partials(nsrc,src_cusp,src_lat0,src_lon0,src_dep0,
    ...
    # Save raytracing to file
    np.savetxt(os.path.join(txtfol,'tt_res_p.txt'),tmp_ttp_res)
    np.savetxt(os.path.join(txtfol,'tt_res_s.txt'),tmp_tts_res)
    with open(os.path.join(txtfol,'tt_sta.txt'),'w') as f:
        for sta in sta_lab:
            f.write('%s \n' % sta)
    with open(os.path.join(txtfol,'tt_ev.txt'),'w') as f:
        for ev in src_cusp:
            f.write('%i \n' % ev)
    np.savetxt(os.path.join(txtfol,'tt_p_loc.txt'),tmp_ttp_old)
    np.savetxt(os.path.join(txtfol,'tt_s_loc.txt'),tmp_tts_old)
    np.savetxt(os.path.join(txtfol,'tt_p_reloc.txt'),tmp_ttp)
    np.savetxt(os.path.join(txtfol,'tt_s_reloc.txt'),tmp_tts)
    ...
    if not iboot and jke==-9 and jks==-9:
        ...
        for i in range(0,nsrc):
            for j in range(0,nsta):
                res.write('%i %s P %3.8f %3.8f %3.8f' % 
                          (src_cusp[i],sta_lab[j],
                           tmp_ttp_old[j,i],tmp_ttp[j,i],tmp_ttp_res[j,i]))
                res.write('%i %s S %3.8f %3.8f %3.8f' % 
                          (src_cusp[i],sta_lab[j],
                           tmp_tts_old[j,i],tmp_tts[j,i],tmp_tts_res[j,i]))
        res.close()
